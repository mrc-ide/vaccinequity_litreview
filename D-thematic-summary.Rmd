...
df_names <- df_names %>% 
  select(covidence_id, first_author_surname, year_of_article, simple_country) %>% 
  distinct() %>%
  mutate(covidence_id = as.numeric(covidence_id))
...
out <- out %>% 
  group_by(simple_country) %>% 
  summarise(out_country = paste0(out_text, collapse = " ")) %>%
  mutate(out_text = paste0(simple_country,": ", out_country))
